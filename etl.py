...
def process_song_data(spark, input_data, output_data):
    ...
    # read song data file
    df = spark.read.json(song_data).load(song_data).dropDuplicates() 
...
def process_log_data(spark, input_data, output_data):
    """This function will ingest the data from the log-data path, will create the parquets files for the users table and the time table. Later, the songplays table, it will be joined with the data from the song_path."""
...
def main():
    """This function will call the spark session and create the dataframes that will be written in parquet in S3."""
